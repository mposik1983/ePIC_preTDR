...
void RecoNhitsvsEta_ePIC_noMPGDs(Double_t mom=1.0)
  {
   ...
   gStyle->SetOptStat(0);
     ...
     // MC Track Properties
    TString InDIR = "/gpfs02/eic/mposik/ePIC/hits/pre-TDRv2/eicrecon/noMPGDs/rootfiles";
    TFile* file = new TFile(Form("%s/eicrecon_v1.30.0_25.10.0_pi-_%0.1fGeV_3deg_177deg_0.0deg_360.0deg_tracking_only_1.podio.root",InDIR.Data(),mom)); // Tree with tracks and hits
																   TFile* ofile = new TFile(Form("./Output/RecoNhitsvsmom_%.1f_noMPGD_hist.root",mom),"recreate");
    ofile->cd();
  ...
  hits->Draw("hist");
  hits->Write();
  c1->SaveAs(Form("./Output/RecoNhitsvsmom%1.1f_NoMPGD.png",mom));
  c1->SaveAs(Form("./Output/RecoNhitsvsmom%1.1f_NoMPGD.root",mom));
  ofile->Close();
}
